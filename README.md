...
独立运行的角色与静态表情创作 WebUI：角色卡、Niji 7 设定图提示词、参考母版、48 张可选反应卡、七类构图、云端/RunningHub 批量生成、逐张重做、中文加字与 ZIP 导出。无需安装 VCP，也无需安装 Codex Skills。
...
4. 在「表情工坊」勾选反应卡，分别编辑构图、动作与文案；默认「百变可爱 12」包含七类构图、仅两张贴脸特写。先选少量不同景别验证画风，再生成整套。
...
每张卡片显示构图类型，已选列表显示整套的构图分布。点开卡片可以换构图；发型、瞳色、饰件与画风保持一致，机位、裁切、姿势和画面占比单独控制。构图改变只影响下一次生成，不会把已有图片假装改成另一种构图。
...
一张表情对应一次图片任务。表情任务必须带角色参考图，优先使用选定的 Q 版母版。母版用于保留身份和画风，不照搬其中的姿势、机位或裁切。参考图失败会明确报错，不降级为纯文生图。请求提交后若网络断开、结果不明，任务进入「状态不明」，不会自动重复提交付费请求。新生成保留旧版本。
