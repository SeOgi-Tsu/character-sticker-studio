...
import type { Asset, Bootstrap, Caption, Catalog, Character, Composition, Job, Project, ProviderSettings, Reaction } from './shared/types';
...
function ReactionCard({ reaction, composition, index, selected, active, job, onSelect, onOpen }: { reaction: Reaction; composition?: Composition; index: number; selected: boolean; active: boolean; job?: Job; onSelect: () => void; onOpen: () => void }) {
...
export default function App() {
  ...
  const [catalog, setCatalog] = useState<Catalog>({ reactions: [], styles: [], packs: [], sources: [], compositions: [] });
  ...
  return <div className={`app-shell ${sidebarOpen ? 'sidebar-open' : ''}`}>
    ...
    <div className="workspace"><header className="topbar"><div className="breadcrumbs"><button className="icon-button mobile-menu" aria-label="打开导航" onClick={() => setSidebarOpen(true)}><Menu size={20} /></button><span>{project.character.name || '新角色'} 的工作室</span><span className="breadcrumb-slash">/</span><strong>{page === 'niji' ? 'Niji 7 角色工坊' : navigation.find(item => item.id === page)?.label}</strong></div><div className="topbar-actions"><span className={`save-state ${dirty ? 'dirty' : ''}`}>{saving ? <LoaderCircle size={12} className="spin" /> : dirty ? <span /> : <Check size={12} />}{saving ? '保存中' : dirty ? '有未保存修改' : '已保存到本机'}</span><button className="button small quiet save-button" disabled={!dirty || saving || busy} onClick={() => void run(async () => { await saveAll(); notify('修改已保存'); })}>保存</button><button className="icon-button" aria-label="导入项目配方" title="导入项目配方" onClick={() => importRef.current?.click()}><Upload size={17} /></button><button className="icon-button" aria-label="导出项目配方" title="导出项目配方" onClick={() => void exportRecipe()}><FileJson size={17} /></button><span className="version-label">LOCAL STUDIO <span>01</span></span></div></header>
      ...
      {page === 'stickers' ? <div className="sticker-layout"><main className="sticker-main"><div className="studio-heading"><div><span className="eyebrow">A COLLECTION OF LITTLE EMOTIONS</span><h1>把心情，<em>变可爱。</em><span className="heading-spark"><Sparkles size={29} strokeWidth={1.4} /></span></h1><p>不止换表情，也换小姿态。特写、全身、道具小剧场，都由你挑。</p></div><button className="edition-note" onClick={() => setSourcesOpen(true)}><span>THE CUTE EDIT</span><strong>2026</strong><small>灵感与选款依据 <ArrowRight size={11} /></small></button></div>
        ...
        <div className="pack-section"><div className="pack-topline"><span className="eyebrow">PICK YOUR MOOD</span><button className="text-button" onClick={() => setSourcesOpen(true)}><BookOpen size={13} />怎么选出这些表情？</button></div><div className="pack-options">{catalog.packs.map((pack, index) => <button className={highlightedPack === pack.id ? 'active' : ''} key={pack.id} title={pack.description} onClick={() => { changeProject({ selectedIds: [...pack.reactionIds] }); setActivePack(pack.id); setCategory('全部'); }}><span className="pack-icon">{index === 0 ? <Heart size={18} /> : index === 1 ? <Sparkles size={18} /> : <Layers3 size={18} />}</span><span><strong>{pack.name}</strong><small>{pack.description}</small></span><span className="pack-count">{pack.reactionIds.length}<small>枚</small></span></button>)}</div></div>
        <section className="composition-mix" aria-label="已选表情构图分布"><div className="composition-mix-heading"><span><Layers3 size={14} />这一套的构图</span><small>{project.selectedIds.length ? `${project.selectedIds.length} 张 · ${usedCompositionCount} 种构图` : '选几张，搭配不同姿态'}</small></div><div className="composition-counts">{compositionCounts.map(item => <span key={item.id} className={`composition-count composition-${item.id} ${item.count ? 'has-count' : ''}`} title={item.description}><i /><span>{item.name}</span><strong>{item.count}</strong></span>)}</div><p>{usedCompositionCount === 1 ? '目前选中的是同一种构图，可点开卡片更换，或试试「混合构图」选集。' : '点开任意卡片，单独选构图、改动作。统一角色，保留每张的小变化。'}</p></section><div className="collection-toolbar"><div className="category-tabs" role="group" aria-label="表情分类">{categories.map(item => <button className={category === item ? 'active' : ''} key={item} onClick={() => setCategory(item)}>{item}{item === '全部' && <span>{reactions.length}</span>}</button>)}</div><div className="search-input"><Search size={15} /><input aria-label="搜索表情" placeholder="找一种心情…" value={search} onChange={e => setSearch(e.target.value)} /></div></div>
        ...
        <div className="reaction-grid">{filtered.map((reaction, index) => <ReactionCard key={reaction.id} reaction={reaction} composition={catalog.compositions.find(item => item.id === (reaction.compositionId || 'halfbody'))} index={index} selected={project.selectedIds.includes(reaction.id)} active={active?.id === reaction.id} job={latestFor(reaction.id)} onSelect={() => toggleSelection(reaction.id)} onOpen={() => openReaction(reaction.id)} />)}</div>{filtered.length === 0 && <div className="empty-state"><Search size={30} /><h3>这份心情还没收录</h3><p>换个词搜索，或添加一个自定义表情。</p><button className="button secondary" onClick={() => setCustomOpen(true)}><Plus size={15} />自定义表情</button></div>}
