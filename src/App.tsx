...
function ReactionCard({ reaction, composition, interaction, index, selected, active, job, onSelect, onOpen }: { reaction: Reaction; composition?: Composition; interaction?: Interaction; index: number; selected: boolean; active: boolean; job?: Job; onSelect: () => void; onOpen: () => void }) {
  ...
  return <article className={`reaction-card tone-${index % 6} ${selected ? 'selected' : ''} ${active ? 'inspected' : ''}`} style={{ animationDelay: `${Math.min(index, 16) * 25}ms` }}>
    <button className={`reaction-art ${job?.asset ? 'has-result checker' : ''}`} onClick={onOpen} aria-label={`编辑 ${reaction.name}`} aria-pressed={active}>
      ...
      <span className={`composition-badge composition-${composition?.id || 'halfbody'}`} title={composition?.description}>{composition?.name || '半身互动'}</span>
      {job && !job.asset && <span className="card-job-status"><Status status={job.status} /></span>}
    </button>
    <div className="reaction-card-bottom"><button className="reaction-card-title" onClick={onOpen}><strong>{reaction.name}</strong>{interaction && interaction.id !== 'observe' ? <span className="card-interaction" title={`${interaction.description}${reaction.intent ? ` 看完想：${reaction.intent}` : ''}`}><i aria-hidden="true">✦</i>{interaction.name} · 张力 {reaction.intensity || 2}</span> : <span>{reaction.tags.slice(0, 2).join(' / ') || reaction.category}</span>}</button><label className="card-checkbox"><input type="checkbox" checked={selected} onChange={onSelect} aria-label={`选择 ${reaction.name}`} /><span><Check size={13} strokeWidth={3} /></span></label></div>
  </article>;
}
...
export default function App() {
  ...
  async function generate(kind: Job['kind'], ids?: string[]) {
    await run(async () => {
      ...
      if (!settings.hasApiKey || (settings.provider !== 'runninghub' && !settings.model) || !settings.baseUrl || (settings.provider === 'runninghub' && !settings.runninghub?.resourceId)) { setSettingsOpen(true); notify('先连接图片 API，再开始生成。', 'error'); return; }
      if (settings.provider === 'runninghub' && kind !== 'character' && !settings.runninghub?.referenceNode) { setSettingsOpen(true); notify('这个 RunningHub 工作流还需要配置角色参考图输入。', 'error'); return; }
  ...
  async function retry(job: Job, confirmed = false) {
    if (canResumeJob(job)) { await resume(job); return; }
  ...
  async function resume(job: Job) {
    await run(async () => {
      const resumed = await post<Job>(`/api/jobs/${job.id}/resume`);
      setJobs(existing => withJobs(existing, [resumed]));
      if (resumed.asset) setAssets(existing => withAssets(existing, [resumed.asset!]));
      notify('正在继续查询原 RunningHub 任务，没有提交新的绘图请求。');
    });
  }
  ...
  const previewRank = new Map(previewOrder.map((id, index) => [id, index]));
  const filtered = reactions.filter(item => (category === '全部' || item.category === category) && [item.name, item.caption, item.action, ...item.tags].join(' ').toLowerCase().includes(search.toLowerCase())).sort((a, b) => (previewRank.get(a.id) ?? previewOrder.length) - (previewRank.get(b.id) ?? previewOrder.length));
  ...
  const compositionCounts = catalog.compositions.map(composition => ({ ...composition, count: reactions.filter(item => project.selectedIds.includes(item.id) && (item.compositionId || 'halfbody') === composition.id).length }));
  const usedCompositionCount = compositionCounts.filter(composition => composition.count > 0).length;
  const interactionCounts = catalog.interactions.map(interaction => ({ ...interaction, count: reactions.filter(item => project.selectedIds.includes(item.id) && (item.interactionId || 'observe') === interaction.id).length }));
  const usedInteractions = interactionCounts.filter(interaction => interaction.count > 0);
  const matchingPacks = catalog.packs.filter(pack => pack.reactionIds.length === project.selectedIds.length && pack.reactionIds.every(id => project.selectedIds.includes(id)));
  const highlightedPack = matchingPacks.find(pack => pack.id === activePack)?.id || matchingPacks[0]?.id;
  ...
  return <div className={`app-shell ${sidebarOpen ? 'sidebar-open' : ''}`}>
    ...
    <div className="workspace"><header className="topbar"><div className="breadcrumbs"><button className="icon-button mobile-menu" aria-label="打开导航" onClick={() => setSidebarOpen(true)}><Menu size={20} /></button><span>{project.character.name || '新角色'} 的工作室</span><span className="breadcrumb-slash">/</span><strong>{page === 'niji' ? 'Niji 7 角色工坊' : navigation.find(item => item.id === page)?.label}</strong></div><div className="topbar-actions"><span className={`save-state ${dirty ? 'dirty' : ''}`}>{saving ? <LoaderCircle size={12} className="spin" /> : dirty ? <span /> : <Check size={12} />}{saving ? '保存中' : dirty ? '有未保存修改' : '已保存到本机'}</span><button className="button small quiet save-button" disabled={!dirty || saving || busy} onClick={() => void run(async () => { await saveAll(); notify('修改已保存'); })}>保存</button><button className="icon-button" aria-label="导入项目配方" title="导入项目配方" onClick={() => importRef.current?.click()}><Upload size={17} /></button><button className="icon-button" aria-label="导出项目配方" title="导出项目配方" onClick={() => void exportRecipe()}><FileJson size={17} /></button><span className="version-label">LOCAL STUDIO <span>01</span></span></div></header>
      ...
      {page === 'stickers' ? <div className="sticker-layout"><main className="sticker-main"><div className="studio-heading"><div><span className="eyebrow">A COLLECTION OF LITTLE EMOTIONS</span><h1>把心情，<em>变可爱。</em><span className="heading-spark"><Sparkles size={29} strokeWidth={1.4} /></span></h1><p>扑过来、求摸摸、一本正经地搞笑。把可爱，递到对方面前。</p></div><button className="edition-note" onClick={() => setSourcesOpen(true)}><span>THE CUTE EDIT</span><strong>2026</strong><small>灵感与选款依据 <ArrowRight size={11} /></small></button></div>
        ...
        <div className="pack-section"><div className="pack-topline"><span className="eyebrow">PICK YOUR MOOD</span><button className="text-button" onClick={() => setSourcesOpen(true)}><BookOpen size={13} />怎么选出这些表情？</button></div><div className="pack-options">{catalog.packs.map((pack, index) => <button className={highlightedPack === pack.id ? 'active' : ''} key={pack.id} title={pack.description} onClick={() => { changeProject({ selectedIds: [...pack.reactionIds] }); setPreviewOrder([...pack.reactionIds]); setActivePack(pack.id); setCategory('全部'); setSearch(''); }}><span className="pack-icon">{index === 0 ? <Heart size={18} /> : index === 1 ? <Sparkles size={18} /> : <Layers3 size={18} />}</span><span><strong>{pack.name}</strong><small>{pack.description}</small></span><span className="pack-count">{pack.reactionIds.length}<small>枚</small></span></button>)}</div></div>
        <section className="composition-mix" aria-label="已选表情的镜头与互动搭配">
          <div className="composition-mix-heading"><span><Layers3 size={14} />镜头 × 互动</span><small className="mix-totals">{project.selectedIds.length ? `${usedCompositionCount} 种构图${catalog.interactions.length ? ` · ${usedInteractions.length} 种互动` : ''}` : '选几张，搭配不同感受'}</small></div>
          <div className="composition-counts">{compositionCounts.map(item => <span key={item.id} className={`composition-count composition-${item.id} ${item.count ? 'has-count' : ''}`} title={item.description}><i /><span>{item.name}</span><strong>{item.count}</strong></span>)}</div>
          <p>{usedInteractions.length ? <><span className="interaction-mix-label">互动搭配</span>{usedInteractions.map(item => `${item.name} ${item.count}`).join(' / ')}<span className="mix-guidance">点开卡片，选镜头、调张力。</span></> : '点开卡片，搭配构图和动作；一套里也留几张安静的可爱。'}</p>
        </section><div className="collection-toolbar"><div className="category-tabs" role="group" aria-label="表情分类">{categories.map(item => <button className={category === item ? 'active' : ''} key={item} onClick={() => setCategory(item)}>{item}{item === '全部' && <span>{reactions.length}</span>}</button>)}</div><div className="search-input"><Search size={15} /><input aria-label="搜索表情" placeholder="找一种心情…" value={search} onChange={e => setSearch(e.target.value)} /></div></div>
