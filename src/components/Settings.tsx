...
export default function Settings({ settings, onSaved, onClose }: { settings: ProviderSettings; onSaved: (value: ProviderSettings) => void; onClose: () => void }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      if (isRunningHub) {
        ...
        checkedRunningHub = { ...runninghub, resourceId: runninghub.resourceId.trim(), promptNode: { nodeId: runninghub.promptNode.nodeId.trim(), fieldName: runninghub.promptNode.fieldName.trim() }, referenceNode: runninghub.referenceNode ? { nodeId: runninghub.referenceNode.nodeId.trim(), fieldName: runninghub.referenceNode.fieldName.trim() } : undefined, styleReferenceNode: runninghub.styleReferenceNode ? { nodeId: runninghub.styleReferenceNode.nodeId.trim(), fieldName: runninghub.styleReferenceNode.fieldName.trim() } : undefined, extraNodes };
      }
  ...
  return <Modal title="连接你的绘图模型" subtitle="选择你的服务所支持的协议，也可以连接 RunningHub 工作流。" onClose={onClose} wide>
    <form onSubmit={save} className="settings-form">
      ...
      {!isRunningHub && <Field label="模型名称" hint="填写服务商的图片模型 ID。原图与 Q 版母版同时使用时需支持多图输入；若不支持，会显示接口错误，不会自动丢弃参考图。"><input required value={form.model} onChange={e => change({ model: e.target.value })} placeholder="来自你的服务商模型列表" autoComplete="off" /></Field>}
      ...
      {isRunningHub && <section className="runninghub-config" aria-label="RunningHub 工作流配置">
        ...
        <div className="node-mapping"><label className="check-label"><input type="checkbox" checked={Boolean(runninghub.referenceNode)} onChange={e => changeRunningHub(e.target.checked ? { referenceNode: { nodeId: '', fieldName: '' } } : { referenceNode: undefined, styleReferenceNode: undefined })} /><strong>② 接入角色参考图</strong></label><p>工作流需实际使用参考图保持角色一致。沿用原图服装时，主节点接收原始角色图。</p>{runninghub.referenceNode && <>
          <div className="form-row"><Field label="参考图节点 ID"><input required value={runninghub.referenceNode.nodeId} onChange={e => changeRunningHub({ referenceNode: { ...runninghub.referenceNode!, nodeId: e.target.value } })} placeholder="从 API 示例复制" autoComplete="off" /></Field><Field label="参考图字段名"><input required value={runninghub.referenceNode.fieldName} onChange={e => changeRunningHub({ referenceNode: { ...runninghub.referenceNode!, fieldName: e.target.value } })} placeholder="从 API 示例复制" autoComplete="off" /></Field></div>
          <label className="check-label"><input type="checkbox" checked={Boolean(runninghub.styleReferenceNode)} onChange={e => changeRunningHub({ styleReferenceNode: e.target.checked ? { nodeId: '', fieldName: '' } : undefined })} /><strong>同时接入 Q 版母版（可选）</strong></label>
          {runninghub.styleReferenceNode ? <><p>第二节点接收母版，用于脸型与画风；与上方角色参考图使用不同的节点或字段。</p><div className="form-row"><Field label="母版节点 ID"><input required value={runninghub.styleReferenceNode.nodeId} onChange={e => changeRunningHub({ styleReferenceNode: { ...runninghub.styleReferenceNode!, nodeId: e.target.value } })} placeholder="从 API 示例复制" autoComplete="off" /></Field><Field label="母版字段名"><input required value={runninghub.styleReferenceNode.fieldName} onChange={e => changeRunningHub({ styleReferenceNode: { ...runninghub.styleReferenceNode!, fieldName: e.target.value } })} placeholder="从 API 示例复制" autoComplete="off" /></Field></div></> : <p>仅发送原始角色图；配置第二节点可同时传入 Q 版母版。</p>}
        </>}</div>
