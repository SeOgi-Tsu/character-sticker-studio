  ...
  return <section className="caption-editor" aria-label="本张表情文字设置">
    ...
    {mode !== 'none' && <>
      ...
      <div className="caption-settings">
        ...
        <Field label={mode === 'generated' ? `字的视觉大小 · 约 ${Math.round(caption.fontSize / 512 * 100)}% 画面` : `字的大小 · 约 ${Math.round(caption.fontSize / 512 * 100)}% 短边`}><input aria-label={mode === 'generated' ? '字的视觉大小' : '文字字号'} type="range" min={20} max={96} value={caption.fontSize} onChange={e => patch({ fontSize: Number(e.target.value) })} /></Field>
